...
from model.evaluate import compute_tile_predictions, custom_spearmanr

def tile_level_quantification(models_dir, output_dir, MFP_dir, histopatho_features_dir, prediction_mode, n_outerfolds, slide_type="FF"):
    """
    ...
    if any([prediction_mode == item for item in ['tcga_train_validation', 'test']]):
        if slide_type == "FFPE":
            ...
            for key, chunk in ffpe_slides.items():
                ...
                for cell_type in cell_types:
                    print('start prediction of cell type', cell_type)
        ...
        elif slide_type == "FF": #if FF slides run through FFPE trained model (this does not happen)
            for cell_type in cell_types:
                print('start prediction of cell type', cell_type)
                cell_type_tile_predictions = compute_tile_predictions(
                    cell_type=cell_type, models_dir=models_dir, n_outerfolds=n_outerfolds,
    ...
    print('Done saving tile predictions')
