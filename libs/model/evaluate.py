...
def custom_spearmanr(x, y, in_gridsearch=True):
    ...
    """
    spearman = stats.spearmanr(x,y,axis=0)[0] #first value statistics, second value p-values
    diag = np.diag(spearman, x.shape[1])
    if in_gridsearch == True:
        return np.mean(abs(diag))
    else:
        diag = np.diag(spearman, x.shape[1])
        return diag
...
def compute_tile_predictions_old(
    cell_type,
    models_dir,
    n_outerfolds,
    prediction_mode,
    ...
    """
    Predict the scores using the trained multi-task lasso models, average cross cell-type signatures and average across folds
    1. prediction_mode=tcga_train_validation: predict for all tiles (validation+train)
    ...
    outer_models = joblib.load(f"{full_model_path}/outer_models.pkl", fix_imports=True)
    x_scalers = joblib.load(f"{full_model_path}/x_train_scaler.pkl", fix_imports=True)

    # Check prediction mode
    if  any([prediction_mode == item for item in ['tcga_train_validation', 'tcga_validation']]): #tcga_train_validation not really needed here 
        outer_fold_splits = joblib.load(f"{full_model_path}/cv_outer_splits.pkl", fix_imports=True)
        total_tile_selection = joblib.load(f"{full_model_path}/total_tile_selection.pkl", fix_imports=True)[["tile_ID", "slide_submitter_id"]]
    ...
    for i in range(n_outerfolds):
        ...
        if any([prediction_mode == item for item in ['tcga_train_validation', 'test']]):
            ...
            #get slide submitter IDs from test slides
        ...
        test_pred[i] = outer_model.predict(X_z).mean(axis=1) #add mean as value to dict 
        index_pred[i] = X_test.index #add tile_ID as value to dict
    ...
    if any([prediction_mode == item for item in ['tcga_train_validation', 'test']]):
        ...
        #combi_scores = np.concatenate((test_pred[0][:,np.newaxis],test_pred[1][:,np.newaxis]),axis=1)
        combi_scores = test_pred[0][:,np.newaxis]
        for n in range(1,n_outerfolds):
            combi_scores = np.concatenate((combi_scores,test_pred[n][:,np.newaxis]),axis=1)
        ...
        #combi_scores = np.concatenate((test_pred[0],test_pred[1]),axis=0)
        combi_scores = test_pred[0]
        combi_indexes = index_pred[0]
        for n in range(1,n_outerfolds):
            combi_scores = np.concatenate((combi_scores,test_pred[n]),axis=0)
            combi_indexes = np.concatenate((combi_indexes,index_pred[n]),axis=0)
...
def compute_tile_predictions(
    ...
    test_pred_missing = dict.fromkeys(range(n_outerfolds))
    index_pred_missing = dict.fromkeys(range(n_outerfolds))
      
    #get all slides and tiles not used for training because target data was missing.
    if prediction_mode == 'tcga_validation':
        all_slides = list(metadata["slide_submitter_id"].unique())
        all_test_slides = []
        #get all slides and tiles
        for i in range(n_outerfolds):
            _, test_index = outer_fold_splits[i]
            test_slides = list(total_tile_selection.loc[test_index, "slide_submitter_id"].unique())
            all_test_slides.extend(test_slides)
        missing_IDs = list(set(all_slides) - set(all_test_slides))
        if slide_type == "FF":
            tiles_missing = metadata[metadata["slide_submitter_id"].isin(missing_IDs)]
            X_missing = X.loc[tiles_missing.tile_ID,:]
        elif slide_type == "FFPE":
            print("X is a Dask DataFrame")
            tiles_missing = metadata[metadata["slide_submitter_id"].isin(missing_IDs)]["tile_ID"]
            X_missing = X.map_partitions(lambda x: x[x.index.isin(tiles_missing)])
            X_missing = X_missing.compute()
     
    #get all test slides and tiles 
    for i in range(n_outerfolds):
        outer_model = outer_models[i]
        if any([prediction_mode == item for item in ['tcga_train_validation', 'test']]):
    	        X_test = X
        ...
        X_z = x_scaler.transform(X_test)
        #if len(X_missing) > 0:
        if prediction_mode == 'tcga_validation' and len(X_missing) > 0:
            X_z_missing = x_scaler.transform(X_missing)

        # Predict tasks for each tile and average their predictions across deconvolution methods
        test_pred[i] = outer_model.predict(X_z).mean(axis=1) #add mean as value to dict 
        index_pred[i] = X_test.index #add tile_ID as value to dict
        
        if prediction_mode == 'tcga_validation' and len(X_missing) > 0:
            test_pred_missing[i] = outer_model.predict(X_z_missing).mean(axis=1)
            index_pred_missing[i] = X_missing.index
    ...
    if any([prediction_mode == item for item in ['tcga_train_validation', 'test']]):
        ...
        if len(X_missing) > 0:
            combi_scores_missing = test_pred_missing[0][:,np.newaxis]
            for n in range(1,n_outerfolds):
                combi_scores_missing = np.concatenate((combi_scores_missing,test_pred_missing[n][:,np.newaxis]),axis=1)
        	    #Average across folds for final prediction
            combi_scores_missing = combi_scores_missing.mean(axis=1)
            combi_indexes_missing = index_pred_missing[1]
    ...
    if any([prediction_mode == item for item in ['tcga_train_validation', 'test']]):
        ...
        if len(X_missing) > 0:
            combi_scores_merged = np.concatenate((combi_scores, combi_scores_missing))
            combi_indexes_merged = np.concatenate((combi_indexes, combi_indexes_missing))
            combi_scores_df = pd.DataFrame(combi_scores_merged)
            combi_scores_df.columns = [f"{cell_type} (combi)"]
            combi_scores_df.index = combi_indexes_merged   
        else:
            combi_scores_df = pd.DataFrame(combi_scores)
            combi_scores_df.columns = [f"{cell_type} (combi)"]
            combi_scores_df.index = combi_indexes   
