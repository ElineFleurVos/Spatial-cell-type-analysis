# -*- coding: utf-8 -*-
"""
Created on Wed Nov 20 10:02:40 2024
...
def calc_average_quantification(slide_data, cell_types=None):
    """
    Calculate the average probabilities for the 4 cell types for a certain slide.

    Parameters
    ----------
    slide_data : dataframe with all tile predictions for a specific slide id
    cell_types : list of cell types

    Returns
    -------
    dataframe containing the average cell type probabilities for a specific slide 
    ...
    average_quantifications = []
    for cell_type in cell_types:
        probs_cell_type = slide_data[cell_type]
        average_prob = sum(probs_cell_type)/len(slide_data)
        
        average_quantifications.append([cell_type, average_prob])
  
    return pd.DataFrame(average_quantifications, columns=["cell_type", "average quantification"])
...
def determine_lcc(graph, cell_type_assignments, cell_types=None):
    ...
    for cell_type in cell_types:
        ...
        graph_test.remove_nodes_from(
            list(cell_type_assignments[~cell_type_assignments[cell_type]].index)
        )
        if len(graph_test.nodes()) == 0:
            print('this slide does not have cell type', cell_type)
        
        for component in components:
            subgraph = graph.subgraph(component)
            
            graph_temp = subgraph.copy()
            ...
            if len(graph_temp.nodes()) > 0:
                ...
                lcc_size = len(max(nx.connected_components(graph_temp), key=len))
                if lcc_size > max_lcc_size:
                    max_lcc_size = lcc_size
                    #best_component = component
                    total_size = len(graph_temp.nodes())
                    print('Max lcc size:', max_lcc_size, 'total_size', total_size)
                    #print('total size:', total_size)
        
        if total_size > 0:
            lcc_frac = max_lcc_size/total_size
...
def compute_n_shortest_paths_max_length(
    ...
    for component_nodes in components:
        ...
        if len(all_paths) > 0:
            ...
            #return pd.DataFrame()
            nr_shortest_paths = shortest_paths.copy()
            nr_shortest_paths["n_paths"]=1
            proximity_graphs = (
                    nr_shortest_paths.groupby(
            ...
            for cell_type in cell_types:
                ...
                dict_count_cell_types[cell_type] = len(graph_temp.nodes)
            
            proximity_graphs['n_total_paths'] = 0
            for i in range(len(proximity_graphs)):
                if proximity_graphs.loc[i, 'source'] == proximity_graphs.loc[i, 'target']:
                    cell_type=proximity_graphs.loc[i, 'source']
                    nr_nodes = dict_count_cell_types[cell_type]
                    nr_total = (nr_nodes*(nr_nodes-1))/2
                    proximity_graphs.loc[i, 'n_total_paths'] = nr_total
                else:
                    cell_type1 = proximity_graphs.loc[i, 'source']
                    cell_type2 = proximity_graphs.loc[i, 'target']
                    nr_nodes1 = dict_count_cell_types[cell_type1]
                    nr_nodes2 = dict_count_cell_types[cell_type2]
                    nr_total = nr_nodes1*nr_nodes2
                    proximity_graphs.loc[i, 'n_total_paths'] = nr_total
                    
            all_proximity_graphs.append(proximity_graphs)
        
        combined_proximity_graphs = pd.concat(all_proximity_graphs, axis=0)
        aggregated_proximity_graphs = (
            combined_proximity_graphs.groupby(["slide_submitter_id", "source", "target"])
            .sum(numeric_only=True)
            .reset_index())  
        
        aggregated_proximity_graphs["pair"] = [f"{source}-{target}" for source, target in proximity_graphs[["source", "target"]].to_numpy()]
        aggregated_proximity_graphs["n_paths_normalized"] = (aggregated_proximity_graphs["n_paths"] / aggregated_proximity_graphs["n_total_paths"])
        
        return aggregated_proximity_graphs
